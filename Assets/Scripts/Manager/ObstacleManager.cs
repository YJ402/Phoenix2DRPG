...
public class ObstacleManager : MonoBehaviour
{
    ...
    //int cellValue = obstacleManager.GetGridValue(x, y);로 가져가기 가능
    //cellValue가 1이면 장애물이 있고, 2면 장애물은 배치하지 못하되 지나갈 수 있고, 3이면 투사체만 통과 가능하게
    ...
    private void MarkArea(int objPosX, int objPosY, int width, int height, GameObject prefab) //장애물의 x, y 좌표 및 장애물 크기
    {
        int obstacleValue = (prefab == water) ? 3 : 1; //프리팹이 물인 경우 obstacleValue는 1이 아닌 3으로 지정됨
        ...
        for (int x = objPosX; x < objPosX + width; x++) //x는 장애물의 x좌표, x좌표 + 오브젝트 사이즈 만큼 반복
        {
            ...
            {
                grid[x, y] = obstacleValue; //장애물이 배치된 곳에는 1을 지정
            }
        }
    ...
    // 그리드 상태 확인 (0: 빈 공간, 1: 장애물, 2: 인접 영역, 3: 물, -1: 유효하지 않은 위치)
